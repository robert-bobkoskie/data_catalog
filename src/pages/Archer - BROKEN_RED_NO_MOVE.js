...
const Archer = () => {
  ...
  const [redFrequency, setRedFrequency] = useState(10000);
  ...
  const fireProjectile = useCallback(() => {
    setProjectiles(prevProjectiles => [...prevProjectiles, { x: 8, y: blackY + 25 / 6 }]);
  }, [blackY]);
  ...
  const checkCollisions = useCallback(() => {
    setProjectiles(prevProjectiles => {
      ...
      const newRedSubMatrices = [...redSubMatrices];
      let newScore = score;

      prevProjectiles.forEach((p) => {
        let hit = false;
        for (let i = 0; i < newRedSubMatrices.length; i++) {
          const r = newRedSubMatrices[i];
          if (p.x + projectileSize > r.x && p.y < r.y + projectileSize && p.y + projectileSize > r.y) {
            newRedSubMatrices.splice(i, 1);
            newScore += 1;
            hit = true;
            break;
          }
  ...
  }, [gameStarted, gameOver, moveDirection, moveBlack, moveProjectiles, moveRedSubMatrices, checkCollisions, checkGameOver]);
